...
namespace SDLFramework {
	...
	void AudioManager::PlaySFX(std::string filename, int loops, int channel) {
		...
		Mix_PlayChannel(channel, mAssetManager->Instance()->GetSFX(filename), loops);
	...
	AudioManager::AudioManager() {
		...
		if (SDL_InitSubSystem(SDL_INIT_AUDIO) < 0) {
			std::cerr << "Unable to in initialize SDL Audio! SLD Error: " << SDL_GetError() << std::endl;
			return;
		}

		// define number of audio channels here
